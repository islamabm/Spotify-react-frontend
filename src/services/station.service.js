...
const API_KEY1 = 'AIzaSyCUFGRnCSVi_ThSlPsSGiian0R5z5i5wWA'
const API_KEY2 = 'AIzaSyCUFGRnCSVi_ThSlPsSGiian0R5z5i5wWA'
const API_KEY3 = 'AIzaSyCUFGRnCSVi_ThSlPsSGiian0R5z5i5wWA'
const apiKeyArr = [API_KEY1, API_KEY2, API_KEY3]
let gApiKey = apiKeyArr[0]
...
async function getCachedVideos(keyword) {
  ...
  let videosIds = storageService.load(VIDEOS_KEY) || []
  const gUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&key=${gApiKey}&q=`
...
async function fetchSong(artist) {
  try {
    const gUrl = `https://www.googleapis.com/youtube/v3/search?part=snippet&key=${gApiKey}&q=`
    const res = await axios.get(gUrl + artist)
    const recommendedSong = res.data.items.map((item) => _prepareData(item))
    ...
    if (err.status !== 403) throw new Error('Not working')
    const arrIndex = apiKeyArr.findIndex((key) => key === gApiKey)
    const newIndex = arrIndex.length < arrIndex + 1 ? 0 : arrIndex + 1
    gApiKey = apiKeyArr[newIndex]
    return await fetchSong(artist)
  }
}
